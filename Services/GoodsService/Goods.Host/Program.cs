...
using Goods.Infrastructure.Modules;
    ...
    class Program
    {
        ...
        static IHostBuilder CreateDefaultHost(string[] args) => new HostBuilder()
           ...
           .UseOxygenService((context, services) =>
           {
               ...
               services.AddDbContext<GoodsContext>(options => options.UseSqlServer(_configuration.GetSection("modules:2:properties:SqlConnectionString").Value));
               services.AddAutofac();
               services.RegisterCapSubscribe();
               services.AddCap(x =>
               {
                   x.UseEntityFramework<GoodsContext>();
